...
:bulb: 他開発者のテスト結果をインポートしたい場合は、事前にテスト結果をエクスポートしたファイルを共有フォルダ等を経由して受け渡し、インポートする側の LatteArt の`latteart-repository/public/imports/` ディレクトリ配下にコピーし、[データインポート](#データインポート)してください。
...
<div align="center">
<img src="./images/generate-test-scripts-icon.png"/> 
</div>
...
- 押下すると現在表示しているテスト結果をエクスポートすることができます。
