...
- テスト実行モデル
  ...
  - テスト結果ごとについているアイコンから以下の操作ができます。
    ...
    | ボタン                                                              | 機能                             |
    | ------------------------------------------------------------------- | -------------------------------- |
    | <div align='center'><img src="images/open-capture-tool.png"/></div> | テスト結果をテスト結果画面に表示 |
...
「[テスト記録のテスト結果画面](/docs/manual/capture/manual-capture.md#テスト結果画面)」と同じようにシーケンス図モデルや画面遷移図モデルで確認ができます。  
レビュー画面でも操作履歴に対する編集は可能となっており、操作一覧の操作を右クリックすることで、気づきやテスト目的を追加することができます。
...
:bulb: 設定情報は、設定画面のプロジェクト設定部分を指します。
...
:bulb: 設定情報は、設定画面で設定したプロジェクト設定を指します。
...
<img src="images/import-user-settings.png" />

「設定ファイルを選択」ボタンを押下するとファイル選択ダイアログが表示され、インポートするファイル(jsonファイル)を選択できます。  
「インポートを実行する」ボタンを押下すると、データがインポートされます。

### ユーザー設定のエクスポート

<img src="images/export-user-settings.png" />

「エクスポートを実行する」ボタンを押下すると、データがエクスポートされます。

- user_settings_YYYYDDMM_HHmmss.json
