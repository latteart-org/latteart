...
- [ヘッダー](#ヘッダー)
  ...
  - [データインポートボタン](#データインポートボタン)
  - [スクリプト生成ボタン](#スクリプト生成ボタン)
  - [データエクスポートボタン](#データエクスポートボタン)
...
テストの開始や終了、既存のテスト結果の読み込み、記録した操作のリプレイなどを行うことができます。
...
押下すると`latteart-repository\public\imports`フォルダ配下の zip ファイル名一覧を取得して表示され、選択したテスト結果をインポートすることができます。  
