...
「自動入力：選択」ダイアログを表示します。有効な自動入力値セットがある場合、活性化します。
...
自動入力の対象となる画面の設定、入力フォームの指定や入力する値等の設定します。
自動入力とは、キャプチャー中に設定した画面を開いた場合、入力フォームに対して値を自動で入力する機能です。

- 入力値を登録するホップアップを自動表示するチェックボックス
  - ON にすると、入力フォームを自動検知し、「自動入力：登録」ダイアログが開きます。
- 入力値を選択するホップアップを自動表示するチェックボックス
...
- 有効/無効（入力値）チェックボックス
  - ON にすると、入力値が有効になります。
- Locator Type
  - 自動入力対象となるフォームの Locator のタイプです。ID と xpath を選択できます。
- Locator Match Type
  - 自動入力対象となるフォームと Locator との一致の仕方を指定します。equals と regex を指定できます。
- Locator
  - 自動入力対象となるフォームの Locator です。ID を指定した場合は ID、xpath を指定した場合は xpath を入力します。（Locator Match Type を regex に指定した場合、正規表現の入力が可能です）
- 入力値
  - 入力フォームに入力する値を設定します。
- 入力値削除ボタン
  - 入力値を削除します。
...
「入力値を登録するホップアップを自動表示する」を ON に設定すると自動で表示されます。
表示タイミングは、キャプチャー中の画面に入力フォームが存在し、その画面から他の画面への遷移時です。
入力値セット名を入力し、OK ボタンを押下すると入力値セットが登録されます。
Locator は対象フォームに ID が設定されていれば ID、設定されていない場合は xpath が登録されます。

### 「自動入力：選択」ダイアログ

<img src="./images/autofill_select_dialog.png"/>

「入力値を選択するホップアップを自動表示する」を ON に設定すると自動で表示されます。
...
- 入力値セット登録ボタン
  - 「自動入力：登録」ダイアログが開きます。該当する行の値を入力値セットとして登録することができます。
