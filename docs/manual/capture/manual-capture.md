...
<img src="./images/search-test-hints.png"/>

<img src="./images/test-hint-search-dialog.png"/>

テスト中に押下するとダイアログが表示され、直近の操作やコメントに近しいテストヒントが一覧で表示されます。  
マッチング結果は（マッチング数/全テストヒント数）で表示されています。  
また、テスト結果がストーリーに紐づいている場合、「現在のストーリーのみ」のチェックボックスをONにすると現在テスト記録中のストーリー（テストマトリクス名、グループ名、テスト対象、観点）に絞り込むことができます。

#### 初期表示時

- マッチング条件表示・非表示ボタン
  マッチング条件変更欄が非表示となっています。ボタンを押下することで表示非表示を変更できます。

  - 表示対象ラジオボタン
    「マッチするテストヒントのみ表示」が選択されています。「全てのテストヒントを表示」を選択するとマッチングが無効となり、全てのテストヒントが表示されます。
  - 画面要素
    画面要素が有効に設定されています。チェックを OFF にすると画面要素のマッチングが無効となります。  
    :bulb:「マッチするテストヒントのみ表示」が選択されている場合に活性状態となります。
  - ヒント本文・キーワード
    ヒント本文・キーワードが有効に設定されています。チェックを OFF にするとヒント本文・キーワード欄が非活性となり、ヒント本文・キーワードのマッチングが無効となります。  
    :bulb:「マッチするテストヒントのみ表示」が選択されている場合に活性状態となります。
  - ヒント本文・キーワード欄
    テストヒント設定を基にコメントを取得し、ヒント本文・キーワード欄に表示します。「自動補完」についての詳細は「[テストヒント設定](../others/manual-config.md/#テストヒント設定)」をご参照ください。  
    :bulb:対象範囲の値を変更するとコメントが存在する場合にマッチングワード欄が更新されます。  
    :bulb:ヒント本文・キーワードが有効の場合に活性状態となります。
  - 対象範囲
    対象範囲(検索時の画面要素・キーワード自動補完範囲)のデフォルト値が設定されています。
  - 現在のストーリーのみ
    現在のストーリーのみが無効に設定されています。チェックを ON にすると現在のストーリーのみが有効となります。  
    :bulb:記録中のテスト結果がストーリーに紐づいている場合に活性状態となります。  
    :bulb:「全てのテストヒントを表示」「マッチするテストヒントのみ表示」どちらが選択されていてもストーリーに紐づいている場合は活性状態となります。

#### 検索条件

「マッチするテストヒントのみ表示」が選択されている場合のみ、以下の検索条件でテストヒントが検索されます。

- 画面要素が有効の場合、対象範囲を基に画面要素を取得し、画面要素と登録済みのテストヒントの画面要素に合致するものを検索します。
- ヒント本文・キーワードが有効の場合、ヒント本文・キーワード欄を基に登録済みのテストヒントを検索します。  
  :warning: ヒント本文・キーワード欄は、半角スペース区切りで設定してください。
- 現在のストーリーのみが有効の場合、現在紐づいているストーリーに合致するものを検索します。

#### 表示順

- 上記検索条件で表示されるテストヒントはヒント本文・キーワード、画面要素の一致数が多い順に降順で表示されます。
- カラムのソートなどで順番は変更可能です。
