...
export class GetFileDataAction {
  public async getFileData(
    payload: { fileUrl: string },
    repositoryService: Pick<RepositoryService, "screenshotRepository">
  ): Promise<ActionResult<Blob | null>> {
    const result = await repositoryService.screenshotRepository.getScreenshotBlobData(
      payload.fileUrl
    );
